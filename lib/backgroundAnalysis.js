  ...
  try {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      try {
        ...
        const RelationshipDetector = require('./relationshipDetector');
        ...
        const queryResult = await askQuestion(dataSource, question.question, {
          timeout: 30000,
    ...
    // Fire webhooks for completion
    const { fireWebhooks } = require('./webhookDelivery');
    const project = db.prepare('SELECT name FROM projects WHERE id = ?').get(projectId);
    fireWebhooks(db, tenantId, 'background_analysis.completed', {
      jobId,
